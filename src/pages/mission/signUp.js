...
export default function Signup() {
  ...
  const confirmId = async () => {
    if (form.id == "") {
      setAlert({ open: true, msg: "아이디를 입력하세요" });
      setIsConfirm(false);
      return;
    }
    ...
    if (data.duplicate) {
      setAlert({ open: true, msg: "중복된 아이디" });
      setIsConfirm(false);
    } else {
      setAlert({ open: true, msg: "사용 가능한 아이디입니다" });
      setIsConfirm(true);
    }
  };
  ...
  const validate = () => {
    if (!form.id || !form.pw || !form.name) {
      setAlert({ open: true, msg: "모든 필드를 입력하세요" });
      return false;
    }
    return true;
  };
  ...
  const register = async () => {
    if (validate()) {
      if (!isConfirm) {
        setAlert({ open: true, msg: "중복 확인을 해주세요" });
        return;
      }
      const { data } = await axios.post("/api/users", {
        ...form,
      ...
      if (data.success) {
        setAlert({
          ...
          msg: "회원가입 성공",
          routerInfo: {
            pathname: "/mission/userInfo",
            query: { user_id: form.id },
          },
        });
      }
    }
          ...
          <div className="md:flex w-full">
            ...
            <div className="w-full md:w-1/2 py-10 px-5 md:px-10">
              <div className="text-center mb-10">
                <h1 className="font-bold text-3xl text-gray-900">회원가입</h1>
              </div>
              <div>
