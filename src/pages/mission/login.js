...
export default function Login() {
  ...
  const IdOnChange = useCallback((e) => setId(e.target.value), []);
  const pwOnChange = useCallback((e) => setPw(e.target.value), []);

  const login = async () => {
    if (!id) {
      setAlert({ open: true, msg: "아이디를 입력해주세요" });
      return;
    }
    if (!pw) {
      setAlert({ open: true, msg: "비밀번호를 입력해주세요" });
      return;
    }

    const { data } = await axios.post("/api/users", { url: "login", id, pw });
    if (data.success) {
      setAlert({ open: true, msg: "로그인 성공" });
    } else {
      setAlert({ open: true, msg: "로그인 실패" });
    }
  };
