...
export default function UserInfo() {
  ...
  const nickNameOnChange = useCallback((e) => setNickName(e.target.value), []);
  const click = useCallback(
    (param) => setGender(param === "M" ? "남자" : "여자"),
    []
  );

  const confirm = async () => {
    if (!nickname) {
      setAlert({ open: true, msg: "입력한 닉네임이 없습니다" });
      setIsConfirm(true);
      return;
    }
    ...
    if (data.duplicate) {
      setAlert({ open: true, msg: "중복된 닉네임입니다" });
      setIsConfirm(false);
    } else {
      setAlert({ open: true, msg: "사용가능 닉네임입니다" });
      setIsConfirm(true);
    }
  };
  ...
  const closeAlert = () => {
    setAlert({ open: false });
    if (alert.routerInfo?.pathname) {
      router.push(alert.routerInfo);
    }
  };

  const skip = () => router.push("/mission/login");

  return (
    <>
      {alert.open && <PropsPopup msg={alert.msg} close={closeAlert} />}
      <div className="min-w-screen min-h-screen bg-gray-900 flex items-center justify-center px-5 py-5">
        <div
          className="bg-gray-100 text-gray-500 rounded-3xl shadow-xl w-full max-w-2xl overflow-hidden"
            ...
            <div className="w-full md:w-1/2 py-10 px-5 md:px-10">
              ...
              <div>
                <div className="flex -mx-3">
                  ...
                  <div className="w-2/5 px-3 mb-5">
                    <label className="px-1"></label>
                    <div className="flex">
                      <button
                        className="w-full py-2 rounded-lg border-2 bg-indigo-500 text-white font-semibold"
                        onClick={confirm}
                      >
                        중복확인
                      </button>
                ...
                <div
                  className="flex -mx-3 mb-5"
                  style={{ flexDirection: "column" }}
                >
                  <label className="text-xs font-semibold px-1 ml-3">
                    사용자 성별
                  </label>
                ...
                <div className="flex -mx-3">
                  ...
                  <div className="w-1/2 px-3 mb-5">
                    <button
                      className="w-full py-2 rounded-lg border-2 bg-gray-300 text-gray-700 font-semibold"
                      onClick={skip}
                    >
                      Skip
                    </button>
